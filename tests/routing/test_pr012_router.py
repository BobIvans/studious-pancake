...
def req():
    return QuoteRequest(SOL, USDC, 1000000, WALLET, 50)


def test_capability_boundary_blocks_assembled_and_quote_only():
    with pytest.raises(TypeError):
        RawInstructionArtifact(ODOS_CAPABILITIES, instructions=(object(),))
    with pytest.raises(TypeError):
        RawInstructionArtifact(OPENOCEAN_CAPABILITIES, instructions=(object(),))
...
def test_okx_hmac_vector_and_params_are_canonical():
    ...
    sig = OkxAuth.sign(
        ...
        "GET",
        "/api/v6/dex/aggregator/swap-instruction?" + query,
    )
    assert sig == "bLuLmyAD4hztejXxgSpcSaJ8NH9/Q1O0lxweiZIc41A="


def test_odos_quote_body_ttl_and_assemble_non_composable():
    clock = FakeClock(datetime(2026, 7, 19, tzinfo=timezone.utc))
    adapter = OdosAdapter(clock=clock)
    body = adapter.quote_body(req())
    assert (
        body["chainId"] == 101
        and body["userAddr"] == WALLET
        and body["inputTokens"][0]["amount"] == "1000000"
    )
    q = adapter.normalize_quote(
        req(), {"pathId": "path-1", "outAmounts": ["123"], "sources": ["Raydium"]}
    )
    ...
    with pytest.raises(TypeError):
        RawInstructionArtifact(art.capabilities, instructions=(object(),))


def test_openocean_missing_key_limiter_unknown_fee_and_dedupe():
    clock = FakeClock(datetime(2026, 7, 19, tzinfo=timezone.utc))
    adapter = OpenOceanAdapter(api_key="key", clock=clock)
    assert (
        adapter.limiter.allow()
        and adapter.limiter.allow()
        and not adapter.limiter.allow()
    )
    oo = adapter.normalize(
        req(),
        {
            "inAmount": "1000000",
            "outAmount": "200",
    ...
    assert (
        oo.provider_fee == "unknown"
        and oo.minimum_output_state is MinimumOutputState.UNPROVEN
    )
    jup = JupiterRouterAdapter(clock=clock).normalize_build(
        req(),
        {
            "inputMint": SOL,
            "outputMint": USDC,
    ...
    assert (
        len(res.discovery_candidates) == 1
        and res.non_selection_reasons[oo2.external_id] is NonSelectionReason.DUPLICATE
    )
...
def test_okx_normalization_is_discovery_only_until_promotion_gate():
            ...
            "instructionLists": [
                {
                    "data": "AQID",
                    "accounts": [
                        {"isSigner": True, "isWritable": True, "pubkey": WALLET}
                    ],
                    "programId": "11111111111111111111111111111111",
                }
            ],
            "addressLookupTableAccount": [WALLET],
            "routerResult": {
                "chainIndex": "501",
    ...
    assert (
        ...
        is NonSelectionReason.NON_COMPOSABLE
    )


def test_selection_uses_conservative_net_not_raw_out():
    clock = FakeClock(datetime(2026, 7, 19, tzinfo=timezone.utc))
    adapter = JupiterRouterAdapter(clock=clock)
    a = adapter.normalize_build(
        req(),
        {
            "inputMint": SOL,
            "outputMint": USDC,
