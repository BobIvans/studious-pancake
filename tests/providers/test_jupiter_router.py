...
def test_schema_preserves_account_meta_flags_and_buckets():
    ...
    assert b.addresses_by_lookup_table_address[
        "ALT1111111111111111111111111111111111111111"
    ][0].startswith("Pool")
    assert b.execution_buckets()["swap"][0].data == b"swap"


def test_rejects_unknown_schema_and_unexpected_tip():
    with pytest.raises(JupiterRouterError) as e:
        parse_build_response(load("router_build_unknown_field_2026-07-19.json"), REQ)
    assert e.value.reason == JupiterRejectionReason.SCHEMA_FAILURE
    with pytest.raises(JupiterRouterError) as e:
        parse_build_response(load("router_build_unexpected_tip_2026-07-19.json"), REQ)
...
async def test_missing_credentials_disable_and_do_not_reserve():
    ...
    with pytest.raises(JupiterRouterError) as e:
        await a.build(None, REQ)  # type: ignore[arg-type]
...
async def test_quota_sliding_window_and_finalization_reserve():
    ...
    with pytest.raises(JupiterQuotaError):
        await qm.reserve("discovery")
    for _ in range(4):
        await qm.reserve("finalization")
    with pytest.raises(JupiterQuotaError):
        await qm.reserve("finalization")
    t[0] = 61
    await qm.reserve("discovery")
...
def test_cu_limit_policy():
    r = SimulationReport(
        ...
        post_account_states=(),
        token_deltas=(),
        native_delta_before_fee=0,
        estimated_network_fee=0,
        simulated_net_profit=None,
        simulation_slot=0,
        min_context_slot=0,
        transaction_message_hash="h",
    )
    ...
    with pytest.raises(JupiterRouterError):
        calculate_final_cu_limit(r3)


def test_two_leg_composition_and_fallback_sequence():
    from src.providers.jupiter.policy import FallbackAction, compose_two_leg_plan, fallback_sequence

    b1 = parse_build_response(load(), REQ)
    req2 = JupiterBuildRequest(
        input_mint=REQ.output_mint,
        output_mint=REQ.input_mint,
        amount=b1.other_amount_threshold,
        taker=REQ.taker,
        slippage_bps=100,
    )
    data = load()
    data["inputMint"] = req2.input_mint
    data["outputMint"] = req2.output_mint
    data["inAmount"] = str(req2.amount)
    b2 = parse_build_response(data, req2)
    plan = compose_two_leg_plan(b1, b2)
    assert [ix.kind for ix in (*plan.setup, *plan.leg1, *plan.leg2, *plan.cleanup)] == [
        "setup",
        "setup",
        "swap",
        "swap",
    ]
    attempts = fallback_sequence(
        "trace-1",
        (64, 56, 50, 48),
        allow_below_50=False,
        direct_route_verified=True,
    )
    assert attempts[0].action == FallbackAction.REMOVE_PROVEN_REDUNDANT_SETUP
    assert [
        attempt.max_accounts
        for attempt in attempts
        if attempt.action == FallbackAction.REBUILD_SMALLER_ACCOUNT_BUDGET
    ] == [56, 50]
    assert all(attempt.max_accounts >= 50 for attempt in attempts)
