...
async def test_reserved_lease_cannot_be_committed_without_issue() -> None:
    ...
    with pytest.raises(ProviderGovernanceError, match="without physical issuance"):
        await authority.complete(lease)
    await authority.release(lease)


@pytest.mark.asyncio
async def test_issued_cancellation_remains_unknown_and_not_released() -> None:
    clock = FakeClock()
...
async def test_dependency_modes_require_health_probe_for_recovery() -> None:
    ...
    controller.activate_generation("provider-a", "generation-2")
    with pytest.raises(ProviderAdmissionError):
        await controller.assert_admissible(
...
async def test_cooldown_preserves_circuit_reason_without_second_call() -> None:
    ...
    registry = ProviderRegistry(
        (adapter,), governance=ProviderGovernance({adapter.provider_id: manifest})
    )
