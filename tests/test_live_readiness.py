...
def policy():
    return load_policy("config/live_risk.yaml")


def store(tmp_path):
    return LiveControlStore(tmp_path / "live.sqlite")


def journal(tmp_path):
    return SQLiteAttemptJournal(tmp_path / "live.sqlite")
...
def test_015_sol_budget_denies(tmp_path):
    p = policy()
    p["live_enabled"] = True
    p["wallet"]["observed_lamports"] = 15_000_000
    s = store(tmp_path)
    j = journal(tmp_path)
...
def test_discovery_only_provider_cannot_execute(tmp_path):
    p = policy()
    p["live_enabled"] = True
    p["providers"]["openocean"]["role"] = "execution"
    s = store(tmp_path)
    j = journal(tmp_path)
    s.arm(canonical_policy_hash(p), 60)
