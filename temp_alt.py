...
class ALTCacheManager:
    ...
    def _parse_alt_data(self, base64_data: str) -> Optional[List[Pubkey]]:
        ...
        try:
            ...
            # that may be missing trailing '=' characters
            padded = base64_data + "=" * (-len(base64_data) % 4)
            data = base64.b64decode(padded)

            # ALT account structure: [header (56 bytes), pubkeys (32 bytes each)]
            # Skip 56-byte header and parse remaining as 32-byte pubkeys
            if len(data) < 56:
                return None

            pubkeys = []
            pubkey_data = data[56:]  # Skip 56-byte header
    ...
    def _get_known_alt_pubkeys(self) -> Set[Pubkey]:
        ...
        These are well-known ALTs that should be cached at startup.
        In practice, this would be dynamically maintained or loaded from config.
        """
        ...
        jupiter_alts = [
            ...
            # Add more Jupiter ALTs as needed
        ]

        # Raydium ALTs
        raydium_alts = [
            # Raydium ALTs would be added here
        ]

        # Orca ALTs
        orca_alts = [
            # Orca ALTs would be added here
        ]

        # Meteora ALTs
        meteora_alts = [
            # Meteora ALTs would be added here
        ]

        # Convert to Pubkey objects
        all_alts = []
        for alt_list in [jupiter_alts, raydium_alts, orca_alts, meteora_alts]:
