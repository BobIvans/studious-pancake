...
class Ranker:
    async def priority(self, opportunity):
        return opportunity.expected_gross_profit


def opportunity(clock: Clock, metadata=None):
    return Opportunity(
        strategy_name="detector",
        opportunity_type="route",
        detected_at=clock.wall,
        detection_slot=7,
        input_mint="mint-a",
        output_mint="mint-b",
        proposed_amount_base_units=10,
        expected_gross_profit=2,
        expires_at=clock.wall + 10,
...
def test_opportunity_is_content_addressed_deeply_immutable_and_strict():
    clock = Clock()
    original = {"route": [{"amount": 10}]}
    first = opportunity(clock, original)
    second = opportunity(clock, {"route": [{"amount": 10}]})
    original["route"][0]["amount"] = 99
    assert first.opportunity_id == second.opportunity_id
    assert first.metadata["route"][0]["amount"] == 10
    with pytest.raises(TypeError):
        first.metadata["route"][0]["amount"] = 2
    for invalid in (True, 1.0):
        with pytest.raises(TypeError):
            Opportunity(
                ...
                input_mint="a",
                output_mint="b",
                proposed_amount_base_units=invalid,
                expected_gross_profit=1,
                expires_at=2,
            )
    with pytest.raises(ValueError):
        opportunity(Clock(float("nan")))


def test_subsecond_ttl_is_preserved_as_integer_milliseconds():
    item = Opportunity.create(
        strategy_name="detector",
        opportunity_type="route",
        detection_slot=7,
        input_mint="mint-a",
    ...
    assert item.expires_at == pytest.approx(100.1)


@pytest.mark.asyncio
async def test_expiry_terminalizes_identity_and_wakes_lifecycle():
    clock = Clock()
    tracker = InMemoryOpportunityTracker(clock=clock)
    queue = OpportunityQueue(2, Ranker(), tracker, clock=clock)
    item = opportunity(clock)
    assert await queue.put(item)
    clock.wall += 11
...
async def test_queue_quiesce_rejects_admission_and_close_is_bounded():
    clock = Clock()
    queue = OpportunityQueue(2, Ranker(), clock=clock)
    await queue.quiesce()
    with pytest.raises(QueueAdmissionError):
        await queue.put(opportunity(clock))
...
async def test_get_creates_owned_lease_and_ack_terminalizes():
    ...
    assert await queue.get(consumer_id="consumer-1") == item
    ...
    assert (
        not queue.leases
        and await tracker.state(item.opportunity_id) is TrackerState.TERMINAL
    )


@pytest.mark.asyncio
async def test_recovery_restores_pending_fence_before_retry_claim():
    clock = Clock()
    tracker = InMemoryOpportunityTracker(clock=clock)
    queue = OpportunityQueue(2, Ranker(), tracker, clock=clock)
    item = opportunity(clock)
    await queue.put(item)
    await queue.get()
    await queue.recover(item)
    assert await tracker.state(item.opportunity_id) is TrackerState.PENDING
    assert await queue.get() == item
    assert queue.leases[0].generation == 2
    assert await tracker.state(item.opportunity_id) is TrackerState.IN_FLIGHT


@pytest.mark.asyncio
async def test_expired_lease_is_recovered_without_an_orphan_reaper():
    clock = Clock()
    tracker = InMemoryOpportunityTracker(clock=clock)
    queue = OpportunityQueue(2, Ranker(), tracker, clock=clock, lease_ns=10)
    item = opportunity(clock)
    await queue.put(item)
    await queue.get()
    clock.mono += 11
    assert await queue.expire() == 0
    assert not queue.leases and queue.qsize() == 1
    assert await tracker.state(item.opportunity_id) is TrackerState.PENDING
...
async def test_tracker_terminal_replay_window_and_capacity_are_bounded():
    clock = Clock()
    tracker = InMemoryOpportunityTracker(capacity=3, replay_ttl_ns=10, clock=clock)
    for item_id in ("a", "b", "c"):
        assert await tracker.mark_pending(item_id)
        await tracker.terminal(item_id)
