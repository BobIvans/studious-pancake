...
_ALLOWED_QUEUE_STATUSES = frozenset(
    {"candidate", "diagnostic-only", "superseded"}
)
...
class AuthorityMap:
    ...
    def _from_raw(
        ...
        *,
        ...
        authorities = tuple(
            AuthorityRecord.from_dict(item)
            for item in _object_list(raw, "authorities")
        )
        verticals = tuple(
            RoadmapVertical.from_dict(item)
            for item in _object_list(raw, "verticals")
        )
        queue = tuple(
            QueueEntry.from_dict(item)
            for item in _object_list(raw, "open_pr_queue")
        )
        _ensure_unique(
        ...
        _ensure_unique(
            (
                ...
                for branch in vertical.active_branches
            ),
            "active branch",
        )
