...
one durable one-shot admission record that a later MPR-2608 boundary may consume.

The caller owns the SQLite transaction.  Methods never commit or roll back.
"""
...
class PrerequisiteIdentity:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "wallet_ref", _nonempty(self.wallet_ref, "wallet_ref"))
...
class HumanPermit:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "prerequisite_digest", _sha(self.prerequisite_digest, "prerequisite_digest")
        )
        object.__setattr__(self, "budget_digest", _sha(self.budget_digest, "budget_digest"))
        object.__setattr__(
            self, "candidate_scope_hash", _sha(self.candidate_scope_hash, "candidate_scope_hash")
        )
...
class DurableCanaryAuthority:
    ...
    second database connection and never commits, preserving the repository's
    existing transaction/fence ownership model.
    """
    ...
    def arm(
        ...
        *,
        ...
        state = self.status()
        if state["latch_code"] is not None or state["outstanding_attempt_id"] is not None:
    ...
    def admit_one_shot(
        ...
        *,
        ...
        if int(state["generation"]) != bundle.attempt_generation:
        ...
        if int(state["arm_expires_at_ms"] or 0) <= now_ms or bundle.deadline_ms <= now_ms:
            raise MPR2609Error("arm/admission expired")
        if bundle.last_valid_block_height - current_block_height < min_blockheight_margin:
        ...
        self._db.execute(
            """INSERT INTO mpr2609_canary_admissions
               (admission_digest, attempt_id, generation, message_digest, bundle_json, consumed)
               VALUES (?, ?, ?, ?, ?, 1)""",
            (digest, bundle.attempt_id, bundle.attempt_generation, bundle.message_digest, payload),
        )
        cursor = self._db.execute(
            """UPDATE mpr2609_canary_control
               ...
               WHERE singleton=1 AND revision=? AND mode=?
                   AND consumed_admission_digest IS NULL AND outstanding_attempt_id IS NULL""",
            (
        ...
        self._event(bundle.attempt_generation, "admitted", {"admission_digest": digest})
        return digest
    ...
    def finalize(
        ...
        *,
        ...
        attempt_id = _nonempty(attempt_id, "attempt_id")
        if isinstance(realized_pnl_lamports, bool) or not isinstance(realized_pnl_lamports, int):
            raise MPR2609Error("realized PnL must be exact integer lamports")
        _sha(reconciliation_digest, "reconciliation_digest")
        _integer(max_cumulative_loss_lamports, "max_cumulative_loss_lamports", 1)
        _integer(max_consecutive_failures, "max_consecutive_failures", 1)
        state = self.status()
        if state["outstanding_attempt_id"] != attempt_id:
            raise MPR2609Error("finalization attempt identity mismatch")

        cumulative_loss = int(state["cumulative_loss_lamports"])
        if realized_pnl_lamports < 0:
            cumulative_loss += -realized_pnl_lamports
        failures = 0 if success else int(state["consecutive_failures"]) + 1
        ...
        self._db.execute(
            """UPDATE mpr2609_canary_control
               ...
               WHERE singleton=1""",
            (mode.value, cumulative_loss, failures, latch.value if latch else None),
        )
        self._event(
    ...
    def rollback_to_shadow(self) -> int:
        ...
        self._db.execute(
            """UPDATE mpr2609_canary_control
               ...
               WHERE singleton=1""",
            (DurableCanaryMode.SHADOW.value, generation),
        )
        self._event(generation, "rollback_to_shadow", {})
        return generation

    def kill(self, *, evidence: str) -> None:
        self._latch(DurableLatch.MANUAL_KILL, _nonempty(evidence, "evidence"))
    ...
    def _latch(self, code: DurableLatch, evidence: str) -> None:
        ...
        self._db.execute(
            ...
            (DurableCanaryMode.LATCHED.value, code.value),
        )
        self._event(int(state["generation"]), "latched", {"code": code.value, "evidence": evidence})

    def _event(self, generation: int, event_type: str, payload: Mapping[str, object]) -> None:
        payload_json = _canonical(dict(payload))
        event_digest = _digest(
            {"generation": generation, "event_type": event_type, "payload": json.loads(payload_json)}
        )
        self._db.execute(
