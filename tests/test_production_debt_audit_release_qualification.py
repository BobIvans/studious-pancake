...
def test_production_debt_audit_overlay_is_non_authoritative_metadata() -> None:
    payload = {
    ...
    assert [item["id"] for item in overlaid["blockers"]] == [
        "runtime.product-state",
        "deployment.image-provenance",
    ]
    assert overlaid["resolved_by_release_qualification"] == []
    assert overlaid["claimed_resolved_by_release_qualification"] == [
        "deployment.image-provenance",
        "runtime.product-state",
    ]
    assert overlaid["production_ready"] is False
    assert overlaid["paper_ready"] is False
    assert overlaid["live_ready"] is False
    assert overlaid["qualification"]["release_id"] == "pytest-release"
    assert overlaid["qualification"]["authoritative"] is False
