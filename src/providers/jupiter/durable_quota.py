...
class DurableJupiterQuotaManager(JupiterQuotaManager):
    ...
    The class subclasses ``JupiterQuotaManager`` only for compatibility with
    existing router/scheduler type expectations. State lives in SQLite, not in
    the inherited in-memory deque/cache.
    """
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            ...
            try:
                # Do not use executescript() inside the explicit transaction.
                # sqlite3.executescript() may issue an implicit COMMIT first when
                # isolation_level=None is used, which breaks the BEGIN/COMMIT
                # invariant that this durable authority relies on.
                connection.execute(
                    """
