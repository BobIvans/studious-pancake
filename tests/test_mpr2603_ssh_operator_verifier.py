...
SAFETY_ROLES = frozenset({"SAFETY_REVIEWER"})
...
def test_real_ssh_signatures_produce_two_independent_verified_approvals(tmp_path: Path) -> None:
    alice_key, alice_pub = _make_key(tmp_path, "alice")
    bob_key, bob_pub = _make_key(tmp_path, "bob")
    registry = _registry(
        _entry(principal="alice-key", subject="human:alice", credential="cred-a", public_key=alice_pub),
        _entry(principal="bob-key", subject="human:bob", credential="cred-b", public_key=bob_pub),
    )
    verifier = SSHOperatorVerifier(registry, ssh_keygen_path=_ssh_keygen())
    request_hash = _request_hash()
    alice = _decision(request_hash=request_hash, principal="alice-key", credential="cred-a")
    ...
    permit = evaluate_human_intervention(request, current_utc="2026-09-09T12:01:00Z")
...
def test_tampered_payload_is_rejected_by_real_backend(tmp_path: Path) -> None:
    ...
    registry = _registry(
        _entry(principal="alice-key", subject="human:alice", credential="cred-a", public_key=public_key)
    )
    verifier = SSHOperatorVerifier(registry, ssh_keygen_path=_ssh_keygen())
    original = _decision(request_hash="6" * 64, principal="alice-key", credential="cred-a")
    signature = _sign(tmp_path, private_key, original)
    tampered = _decision(request_hash="7" * 64, principal="alice-key", credential="cred-a")
...
def test_wrong_namespace_cannot_be_constructed() -> None:
    with pytest.raises(OperatorVerificationError, match="namespace mismatch"):
        _decision(request_hash="6" * 64, principal="alice-key", credential="cred-a", namespace="file")


def test_empty_role_policy_is_rejected_before_crypto(tmp_path: Path) -> None:
    private_key, public_key = _make_key(tmp_path, "alice")
    decision = _decision(request_hash="6" * 64, principal="alice-key", credential="cred-a")
    verifier = SSHOperatorVerifier(
        _registry(_entry(principal="alice-key", subject="human:alice", credential="cred-a", public_key=public_key)),
        ssh_keygen_path=_ssh_keygen(),
    )
    with pytest.raises(OperatorVerificationError, match="ROLE_POLICY_MISSING"):
        verifier.verify(
            decision,
...
def test_unknown_revoked_wrong_role_and_credential_fail_closed(tmp_path: Path) -> None:
    private_key, public_key = _make_key(tmp_path, "alice")
    request_hash = "6" * 64
    decision = _decision(request_hash=request_hash, principal="alice-key", credential="cred-a")
    signature = _sign(tmp_path, private_key, decision)

    unknown = SSHOperatorVerifier(
        _registry(_entry(principal="other", subject="human:other", credential="cred-x", public_key=public_key)),
        ssh_keygen_path=_ssh_keygen(),
    )
    with pytest.raises(OperatorVerificationError, match="UNKNOWN_PRINCIPAL"):
        unknown.verify(decision, signature=signature, current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)

    revoked = SSHOperatorVerifier(
        _registry(_entry(principal="alice-key", subject="human:alice", credential="cred-a", public_key=public_key, active=False)),
        ssh_keygen_path=_ssh_keygen(),
    )
    with pytest.raises(OperatorVerificationError, match="PRINCIPAL_REVOKED"):
        revoked.verify(decision, signature=signature, current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)

    wrong_role = SSHOperatorVerifier(
        _registry(_entry(principal="alice-key", subject="human:alice", credential="cred-a", public_key=public_key, roles=["OBSERVER"])),
        ssh_keygen_path=_ssh_keygen(),
    )
    with pytest.raises(OperatorVerificationError, match="REQUIRED_ROLE_MISSING"):
        wrong_role.verify(decision, signature=signature, current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)

    mismatch = _decision(request_hash=request_hash, principal="alice-key", credential="cred-other")
    ...
    with pytest.raises(OperatorVerificationError, match="CREDENTIAL_MISMATCH"):
        active.verify(mismatch, signature=mismatch_signature, current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)


def test_two_keys_for_same_enrolled_human_do_not_satisfy_independence(tmp_path: Path) -> None:
    key_a, pub_a = _make_key(tmp_path, "alice-a")
    key_b, pub_b = _make_key(tmp_path, "alice-b")
    registry = _registry(
        _entry(principal="alice-a", subject="human:alice", credential="cred-a", public_key=pub_a),
        _entry(principal="alice-b", subject="human:alice", credential="cred-b", public_key=pub_b),
    )
    verifier = SSHOperatorVerifier(registry, ssh_keygen_path=_ssh_keygen())
    request_hash = _request_hash()
    ...
    a1 = verifier.verify(d1, signature=_sign(tmp_path, key_a, d1), current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)
    a2 = verifier.verify(d2, signature=_sign(tmp_path, key_b, d2), current_utc="2026-09-09T12:01:00Z", required_roles=SAFETY_ROLES)
...
def test_registry_rejects_duplicate_keys_unknown_fields_and_nan() -> None:
    ...
    duplicate = (
        '{"schema_version":"%s","trust_epoch":"%s","trust_epoch":"%s","principals":[]}'
            ...
            json.dumps(
                {
                    ...
                    "principals": [
                        {
                            **_entry(principal="p", subject="s", credential="c", public_key=public),
                            "client_role": "ADMIN",
                        }
                    ],
                }
