...
This module does not start discovery, simulation, senders, or live trading.  It
only validates the evidence package that must be produced by the durable
paper/shadow vertical before a human can promote the system toward canary work.
"""
...
def _require_signed_int(value: int, *, field: str) -> int:
...
def _require_sha256(value: str, *, field: str) -> str:
    ...
    if not _SHA256_RE.fullmatch(lowered) or lowered == "0" * 64:
        raise ShadowSoakError(f"{field} must be a non-placeholder sha256 hex digest")
    return lowered
...
def _require_relative_path(value: str, *, field: str) -> str:
    ...
    if (
        ...
        raise ShadowSoakError(f"{field} must be a normalized repository-relative path")
    return normalized
...
class SoakArtifactReference:
    ...
    def __post_init__(self) -> None:
        ...
        if self.event_count is not None:
            _require_non_negative_int(self.event_count, field="artifact.event_count")
    ...
    def __post_init__(self) -> None:
        for item in fields(self):
            ...
            if item.name == "net_pnl_lamports":
                _require_signed_int(value, field=item.name)
            else:
                _require_non_negative_int(value, field=item.name)
        if not (
            self.max_latency_ms >= self.p95_latency_ms >= self.p50_latency_ms
        ):
...
class ReplayEvidence:
    ...
    def __post_init__(self) -> None:
        for item in (
            ...
            _require_non_negative_int(getattr(self, item), field=item)
        object.__setattr__(
            self,
            "corpus_sha256",
        ...
        if (
            ...
            raise ShadowSoakError("deterministic replay counts must sum to replayed events")
    ...
    def pass_rate_bps(self) -> int:
        if self.replayed_events == 0:
            return 0
        return (
            self.deterministic_passed_events * _BPS_DENOMINATOR
        ) // self.replayed_events
...
class ShadowSoakEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        _require_timezone(self.started_at, field="started_at")
        _require_timezone(self.ended_at, field="ended_at")
...
def evaluate_shadow_soak(
    ...
    """Evaluate a PR-060 soak package without enabling runtime or live trading."""
    ...
    observed_stages = set(evidence.vertical_stages)
    ...
    for stage in policy.required_vertical_stages:
        check(stage in observed_stages, f"REQUIRED_STAGE_MISSING:{stage}")
    check(
    ...
    check(
        metrics.reconciliation_mismatches
        <= policy.max_reconciliation_mismatches,
    ...
    check(
        SoakArtifactKind.RAW_EVENTS in artifact_kinds,
        "RAW_EVENT_ARTIFACT_MISSING",
    )
    check(
        SoakArtifactKind.REPLAY_CORPUS in artifact_kinds,
        "REPLAY_CORPUS_ARTIFACT_MISSING",
    )
    check(
        SoakArtifactKind.METRICS_REPORT in artifact_kinds,
    ...
    if policy.require_human_review:
        ...
        check(
            SoakArtifactKind.OPERATOR_REVIEW in artifact_kinds,
...
def to_pr047_shadow_soak_reference(
    ...
    *,
    ...
    """Convert PR-060 evidence into the existing PR-047 PR039 evidence slot."""
