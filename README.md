...
| Paper trading | `available, fail-closed` | `flashloan-bot run --mode paper` / `flashloan-bot paper-shadow --json` используют sender-free paper/shadow runner и возвращают `blocked`/`degraded`, пока upstream evidence неполный |
...
Если README и capability matrix расходятся, считать source of truth матрицу и
исправлять документацию отдельным PR.
...
- `paper` — запускает тот же sender-free PR-089 paper/shadow composition root; не
  отправляет transaction и fail-closed возвращает `blocked`/`degraded`, если
  upstream evidence отсутствует либо неполный.
...
## Post-merge release-gate status

PR-078, PR-079, PR-080 и PR-081 добавили review/evidence boundaries для
security/SBOM/chaos artifacts, real shadow-soak package, canonical sender
lifecycle integration и limited canary release package. Эти PR **не включают live
trading**, не импортируют signer, не мутируют wallet, не отправляют RPC/Jito
transactions и не делают автоматическое canary promotion.

Даже если PR-081 evidence package проходит, результат означает только
`ready-for-manual-canary-review`. Репозиторий не стал автоматическим live bot:
оператор всё ещё должен предоставить реальные артефакты, пройти human review,
сохранить rollback-to-shadow и включать canary только отдельным ручным действием.
