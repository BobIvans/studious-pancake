...
def test_numeric_verticals_have_one_active_branch_and_live_is_hard_disabled():
    ...
    active = {
        item.roadmap_pr: item.active_branches[0]
        for item in authority.verticals
        if item.active_branches
    }
    assert active == {
    ...
    assert all(not item.active_branches for item in authority.verticals[7:])
