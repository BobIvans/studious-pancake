...
class InMemoryOpportunityTracker:
    def __init__(self, *, capacity: int = 10_000, replay_ttl_ns: int = 3_600_000_000_000,
                 clock: TrustedTime | None = None) -> None:
        if isinstance(capacity, bool) or not isinstance(capacity, int) or capacity < 1:
            raise ValueError("tracker capacity must be a positive integer")
        if isinstance(replay_ttl_ns, bool) or not isinstance(replay_ttl_ns, int) or replay_ttl_ns < 1:
    ...
    async def mark_pending(self, opportunity_id: str) -> bool:
        async with self._lock:
            now = self.clock.snapshot().monotonic_ns; self._compact_locked(now)
            if opportunity_id in self._states:
                return False
            if len(self._states) >= self.capacity:
                # Never evict owned work to admit new work.
                terminal = next((k for k, v in self._states.items() if v.state not in {TrackerState.PENDING, TrackerState.IN_FLIGHT}), None)
                if terminal is None:
                    return False
    ...
    async def claim(self, opportunity_id: str, generation: int = 0) -> bool:
        async with self._lock:
            ...
            if state is not None and state.state is not TrackerState.PENDING:
                return False
            self._states[opportunity_id] = TrackerRecord(TrackerState.IN_FLIGHT, now, generation)
            self._states.move_to_end(opportunity_id)
            return True

    async def release_pending(self, opportunity_id: str) -> None:
        async with self._lock:
            if (record := self._states.get(opportunity_id)) and record.state is TrackerState.PENDING:
                self._states.pop(opportunity_id)

    async def terminal(self, opportunity_id: str, state: TrackerState = TrackerState.TERMINAL) -> None:
        ...
        async with self._lock:
            ...
            generation = self._states.get(opportunity_id, TrackerRecord(state, now)).generation
            self._states[opportunity_id] = TrackerRecord(state, now, generation)
            self._states.move_to_end(opportunity_id); self._compact_locked(now)
    ...
    def _compact_locked(self, now: int) -> None:
        expired = [key for key, record in self._states.items() if record.state not in {TrackerState.PENDING, TrackerState.IN_FLIGHT} and now - record.touched_ns >= self.replay_ttl_ns]
        for key in expired: self._states.pop(key, None)
        while len(self._states) > self.capacity:
            key = next((k for k, v in self._states.items() if v.state not in {TrackerState.PENDING, TrackerState.IN_FLIGHT}), None)
