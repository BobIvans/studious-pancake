...
def _closure_map(repo: Path) -> dict[str, Any]:
    ...
    if not isinstance(policy, Mapping) or not policy.get("materialized_runtime_evidence_required"):
        blockers.append("missing-materialized-evidence-policy")
    if not isinstance(policy, Mapping) or not policy.get("offline_validators_do_not_close_blockers"):
    ...
    for row in gates:
        ...
        if row.get("status") == "implemented-offline-validator":
            ...
            elif not (repo / module_path).is_file():
                blockers.append(f"{gate_id}:module-not-found")
    blockers.extend(f"missing-gate:{gid}" for gid in sorted(set(REQUIRED_GATE_IDS) - set(gate_ids)))
    blockers.extend(f"duplicate-gate:{gid}" for gid in sorted({gid for gid in gate_ids if gate_ids.count(gid) > 1}))
    return {
        "present": True,
