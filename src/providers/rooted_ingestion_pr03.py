    ...
    def pending_work_items(
        ...
        *,
        ...
        for row in rows:
            ...
            if not all(bindings):
                raise ProviderVerificationError("PR03_STORED_EVIDENCE_BINDING_MISMATCH")
            issued_at_raw = payload.get("issued_at_ns")
            expires_at_raw = payload.get("expires_at_ns")
            if not isinstance(issued_at_raw, int) or not isinstance(
                expires_at_raw, int
            ):
                raise ProviderVerificationError("PR03_STORED_EVIDENCE_TIME_INVALID")
...
class A3RootedProviderBatchSource:
    ...
    def __call__(self):
        ...
        try:
            ...
            evidence_hash = _hash_json({"schema": PR03_BATCH_SCHEMA, "blocker": reason})
        ...
        identities = {(item.release_id, item.policy_bundle_hash) for item in items}
        if len(identities) != 1:
