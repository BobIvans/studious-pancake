...
class OpportunityQueue:
    def __init__(
        ...
        *,
        ...
        self._retry_generations: dict[str, int] = {}
        self._seq = 0
        self._cv = asyncio.Condition()
        self.state = QueueState.OPEN
        self.metrics: defaultdict[str, StrategyQueueMetrics] = defaultdict(
            StrategyQueueMetrics
        )
    ...
    @property
    def leases(self) -> tuple[InFlightLease, ...]:
    ...
    async def put(self, opportunity: Opportunity) -> bool:
        ...
        async with self._cv:
            if self.state is not QueueState.OPEN:
                raise QueueAdmissionError(self.state)
            await self._recover_expired_leases_locked()
            await self._expire_locked()
            m = self.metrics[opportunity.strategy_name]
            m.detected += 1
            m.last_event = "detected"
            if (
                opportunity.opportunity_id in self._ids
                or opportunity.opportunity_id in self._leases
            ):
                m.duplicates += 1
                m.last_event = "duplicate"
                return False
            if self.tracker is not None and not await self.tracker.mark_pending(
                opportunity.opportunity_id
            ):
                m.duplicates += 1
                m.last_event = "duplicate_lifecycle"
                return False
            item = (
                -float(priority),
        ...
        async with self._cv:
            while True:
                ...
                if self._heap:
                    ...
                    if self.tracker is not None and not await self.tracker.claim(
                        oid, generation
                    ):
                    ...
                    self._leases[oid] = InFlightLease(
                        oid,
                        consumer_id,
                        generation,
                        acquired,
                        acquired + self.lease_ns,
                        2,
                        opp,
                    )
                    return opp
                if self.state in {
                    ...
                    raise QueueClosed(self.state.value)
                await self._cv.wait()
    ...
    async def acknowledge(self, item_id: str, outcome: str) -> None:
        async with self._cv:
            ...
            if lease is None:
                raise ValueError("item has no active lease")
            if self.tracker:
                await self.tracker.terminal(item_id)
            ...
            if quarantine or lease.generation >= lease.retry_limit:
                if self.tracker:
                    await self.tracker.terminal(
                        item.opportunity_id, TrackerState.QUARANTINED
                    )
            elif item.expires_at <= self.clock.snapshot().utc.timestamp():
                if self.tracker:
                    await self.tracker.terminal(
                        item.opportunity_id, TrackerState.EXPIRED
                    )
            else:
                self._retry_generations[item.opportunity_id] = lease.generation
                if self.tracker and not await self.tracker.recover_pending(
                    item.opportunity_id, lease.generation
                ):
                    self.state = QueueState.FAILED
                    self._cv.notify_all()
                    raise RuntimeError("lease recovery lifecycle fence mismatch")
                heapq.heappush(
                    self._heap,
                    (0.0, item.expires_at, self._seq, item.opportunity_id, item),
                )
                self._seq += 1
                self._ids.add(item.opportunity_id)
            self._cv.notify_all()

    async def expire(self) -> int:
        async with self._cv:
            await self._recover_expired_leases_locked()
            return await self._expire_locked()

    async def _recover_expired_leases_locked(self) -> int:
        now = self.clock.snapshot()
        expired = tuple(
            lease
            for lease in self._leases.values()
            if lease.deadline_monotonic_ns <= now.monotonic_ns
        )
        for lease in expired:
            self._leases.pop(lease.item_id, None)
            if lease.item.expires_at <= now.utc.timestamp():
                if self.tracker:
                    await self.tracker.terminal(lease.item_id, TrackerState.EXPIRED)
                self.expiry_events.append((lease.item_id, "lease_expired"))
            elif lease.generation >= lease.retry_limit:
                if self.tracker:
                    await self.tracker.terminal(lease.item_id, TrackerState.QUARANTINED)
            else:
                if self.tracker and not await self.tracker.recover_pending(
                    lease.item_id, lease.generation
                ):
                    self.state = QueueState.FAILED
                    raise RuntimeError("expired lease lifecycle fence mismatch")
                self._retry_generations[lease.item_id] = lease.generation
                heapq.heappush(
                    self._heap,
                    (0.0, lease.item.expires_at, self._seq, lease.item_id, lease.item),
                )
                self._seq += 1
                self._ids.add(lease.item_id)
        if expired:
            self._cv.notify_all()
        return len(expired)

    async def _expire_locked(self) -> int:
        now = self.clock.snapshot().utc.timestamp()
        keep = []
        count = 0
        for item in self._heap:
            if item[1] <= now:
                count += 1
                oid, opp = item[3], item[4]
                self._ids.discard(oid)
                if self.tracker:
                    await self.tracker.terminal(oid, TrackerState.EXPIRED)
                m = self.metrics[opp.strategy_name]
                m.expired += 1
                m.last_event = "expired"
    ...
    async def quiesce(self) -> None:
        async with self._cv:
            if self.state is QueueState.OPEN:
                self.state = QueueState.QUIESCING
            self._cv.notify_all()
    ...
    async def close(self, *, deadline_monotonic_ns: int) -> tuple[str, ...]:
        async with self._cv:
            ...
            while (
                self._leases
                and self.clock.snapshot().monotonic_ns < deadline_monotonic_ns
            ):
                await self._recover_expired_leases_locked()
                if not self._leases:
                    break
                now_ns = self.clock.snapshot().monotonic_ns
                wake_ns = min(
                    deadline_monotonic_ns,
                    *(lease.deadline_monotonic_ns for lease in self._leases.values()),
                )
                remaining = (wake_ns - now_ns) / 1_000_000_000
                try:
                    await asyncio.wait_for(self._cv.wait(), max(0, remaining))
                except TimeoutError:
                    break
            unresolved = tuple(sorted((*self._ids, *self._leases)))
            for _, _, _, oid, _ in self._heap:
                if self.tracker:
                    await self.tracker.terminal(oid, TrackerState.QUARANTINED)
            self._heap.clear()
            self._ids.clear()
