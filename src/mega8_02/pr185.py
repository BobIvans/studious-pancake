...
def collect_protocol_owned_liquidity(
    ...
    return tuple(sorted(rows, key=lambda item: (
        item.protocol_id, item.asset_id, item.event_kind
    )))
...
def collect_fee_sweep_events(
    ...
    return tuple(
        event for event in collect_protocol_owned_liquidity(events, now=now)
        if event.event_kind == "fee_sweep"
    )


def estimate_inventory_release_impact(
    event: InventoryEvent, *, depth: int
) -> int:
    if depth <= 0:
...
def emit_protocol_inventory_candidate(
    ...
    return stable_hash(
        {"protocol": event.protocol_id, "asset": event.asset_id,
         "amount": event.amount, "impact_ppm": impact_ppm,
