...
class ComponentCapability:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "ComponentCapability":
        ...
        if not modes:
            raise CapabilityContractError(f"allowed_modes must not be empty: {raw['id']}")
        ...
        return cls(
            ...
            registry_name=(
                str(raw["registry_name"]) if raw.get("registry_name") is not None else None
            ),
        )
...
class CapabilityMatrix:
    ...
    def load(cls, path: str | Path) -> "CapabilityMatrix":
        ...
        try:
            ...
            raise CapabilityContractError(f"capability matrix not found: {source}") from exc
        except json.JSONDecodeError as exc:
            raise CapabilityContractError(f"invalid capability matrix JSON: {source}: {exc}") from exc
        ...
        components = tuple(ComponentCapability.from_dict(item) for item in components_raw)
        ids = [item.id for item in components]
        if len(ids) != len(set(ids)):
            raise CapabilityContractError("capability matrix contains duplicate component ids")
        registry_names = [
            item.registry_name for item in components if item.registry_name is not None
        ]
        if len(registry_names) != len(set(registry_names)):
            raise CapabilityContractError("capability matrix contains duplicate registry_name values")
    ...
    def load_default(cls) -> "CapabilityMatrix":
        return cls.load(Path(__file__).resolve().parents[1] / "config" / "capabilities.json")
    ...
    def validate_paths(self, root: str | Path | None = None) -> tuple[str, ...]:
        repo_root = Path(root).resolve() if root is not None else self.source_path.parents[1]
        errors = []
        for component in self.components:
            if not (repo_root / component.path).exists():
                errors.append(f"missing component path: {component.id}: {component.path}")
        return tuple(errors)
    ...
    def validate_strategy_registry(self, strategies: Iterable[Any]) -> tuple[str, ...]:
        ...
        for missing in sorted(set(registered) - set(declared)):
            errors.append(f"registered strategy missing from capability matrix: {missing}")
