...
class ResearchEpisode:
    ...
    def __post_init__(self) -> None:
        ...
        feature_at = require_int(
            self.feature_available_at, "feature_available_at", minimum=0
        )
        label_at = require_int(self.label_available_at, "label_available_at", minimum=0)
...
def form_independent_episodes(
    ...
    *,
    ...
    for row in sorted(rows, key=lambda item: int(item["feature_available_at"])):
        feature_at = require_int(
            row.get("feature_available_at"), "feature_available_at", minimum=0
        )
        ...
        episode = ResearchEpisode(
            ...
            label_available_at=require_int(
                row.get("label_available_at"), "label_available_at", minimum=0
            ),
            feature_atoms=require_int(row.get("feature_atoms"), "feature_atoms"),
            label_atoms=require_int(row.get("label_atoms"), "label_atoms"),
        )
...
def compare_four_model_variants(
    *,
    ...
    mechanism_prediction = require_int(
        mechanism_prediction_atoms, "mechanism_prediction_atoms"
    )
    metrics = {
        "simple_baseline_mae_atoms": _mae(holdout, simple_prediction),
        "local_only_mae_atoms": _mae(holdout, local_prediction),
        "pooled_markets_mae_atoms": _mae(holdout, pooled_prediction),
        "mechanism_transfer_mae_atoms": _mae(holdout, mechanism_prediction),
    }
...
def evaluate_probability_calibration(
    ...
    covered = 0
    for row in rows:
        probability = require_ppm(row.get("probability_ppm"), "probability_ppm")
        outcome = require_int(row.get("outcome"), "outcome", minimum=0)
        ...
        lower = require_ppm(row.get("interval_low_ppm"), "interval_low_ppm")
        upper = require_ppm(row.get("interval_high_ppm"), "interval_high_ppm")
        if upper < lower:
...
def source_value_report(
    *,
    ...
    qualified = require_int(
        qualified_candidates, "qualified_candidates", minimum=0
    )
    engineering = require_int(
        engineering_cost_units, "engineering_cost_units", minimum=0
    )
    data = require_int(data_cost_units, "data_cost_units", minimum=0)
    license_cost = require_int(
        license_cost_units, "license_cost_units", minimum=0
    )
    failures = require_int(source_failures, "source_failures", minimum=0)
