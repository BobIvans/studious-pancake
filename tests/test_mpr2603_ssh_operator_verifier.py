...
def _request_hash() -> str:
    ...
    # Constructor intentionally requires approvals; compute the same canonical
    # hash locally so signatures can be produced before approvals exist.
    payload = {
...
def test_tampered_payload_is_rejected_by_real_backend(tmp_path: Path) -> None:
    ...
    with pytest.raises(OperatorVerificationError, match="SIGNATURE_INVALID"):
        verifier.verify(tampered, signature=signature, current_utc="2026-09-09T12:01:00Z")
...
def test_unknown_revoked_wrong_role_and_credential_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(OperatorVerificationError, match="UNKNOWN_PRINCIPAL"):
        unknown.verify(decision, signature=signature, current_utc="2026-09-09T12:01:00Z")
    ...
    with pytest.raises(OperatorVerificationError, match="PRINCIPAL_REVOKED"):
        revoked.verify(decision, signature=signature, current_utc="2026-09-09T12:01:00Z")
    ...
    with pytest.raises(OperatorVerificationError, match="CREDENTIAL_MISMATCH"):
        active.verify(mismatch, signature=mismatch_signature, current_utc="2026-09-09T12:01:00Z")
...
def test_two_keys_for_same_enrolled_human_do_not_satisfy_independence(tmp_path: Path) -> None:
    ...
    a1 = verifier.verify(d1, signature=_sign(tmp_path, key_a, d1), current_utc="2026-09-09T12:01:00Z")
    a2 = verifier.verify(d2, signature=_sign(tmp_path, key_b, d2), current_utc="2026-09-09T12:01:00Z")
