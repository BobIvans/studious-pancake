...
class JupiterRouterAdapter(ProviderAdapter):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # PR-031: Jupiter must not have a private FixedWindowLimiter here.  The
        # same manager instance is passed to discovery, route refinement and
        # final build/finalization so those stages cannot independently spend
        # the account/API-key quota.
    ...
    def normalize_build(self, req: QuoteRequest, payload: dict[str, Any]) -> NormalizedQuote:
        if (
            payload.get("inputMint") != req.input_mint
            or payload.get("outputMint") != req.output_mint
            or str(payload.get("inAmount")) != str(req.amount_base_units)
        ):
            raise ValueError("jupiter response does not match request")
        out = int(payload["outAmount"])
        min_out = int(payload["otherAmountThreshold"])
        route = tuple(
        ...
        return NormalizedQuote(
            ...
            req.fingerprint,
            raw_hash(payload),
            payload.get("requestId") or raw_hash(payload)[:16],
            req.input_mint,
            req.output_mint,
            req.amount_base_units,
            out,
            min_out,
            MinimumOutputState.PROVEN,
            req.swap_mode,
            int(payload.get("slippageBps", req.slippage_bps)),
            route,
            route,
            payload.get("priceImpactPct"),
            str(payload.get("platformFee")) if payload.get("platformFee") is not None else None,
            None,
            payload.get("contextSlot"),
            self.clock.now(),
...
class OkxDexAdapter(ProviderAdapter):
        ...
        **kwargs: Any,
        ...
        if not all((api_key, passphrase, secret)):
            self.capabilities = replace(
                ...
                admission_reason="disabled_missing_credentials: OKX key/passphrase/secret required",
            )
    ...
    def build_params(self, req: QuoteRequest) -> dict[str, str]:
        return {
            ...
            "slippagePercent": str(req.slippage_bps / 100),
        }
    ...
    def normalize(self, req: QuoteRequest, payload: dict[str, Any]) -> NormalizedQuote:
        ...
        router_result = data.get("routerResult") or {}
        if (
            router_result.get("chainIndex") != "501"
            or str(router_result.get("fromTokenAmount")) != str(req.amount_base_units)
        ):
            raise ValueError("okx response does not match request")
        out = int(router_result["toTokenAmount"])
        min_out = int((router_result.get("tx") or {}).get("minReceiveAmount"))
        for instruction in data.get("instructionLists", []):
            ...
            for account in instruction.get("accounts", []):
                ...
                assert isinstance(account.get("isSigner"), bool) and isinstance(
                    account.get("isWritable"), bool
                )
        route = tuple(
            pool.get("dexProtocol", [{}])[0].get("dexName", "okx")
            for route_item in router_result.get("dexRouterList", [])
            for pool in route_item.get(
                "subRouterList",
                route_item.get("router", []) if isinstance(route_item.get("router"), list) else [],
            )
        ) or ("okx",)
        return NormalizedQuote(
            self.provider_id,
            ...
            route,
            route,
            router_result.get("priceImpactPercent"),
            router_result.get("tradeFee") or router_result.get("estimateGasFee"),
            None,
            None,
            self.clock.now(),
            self.clock.now() + timedelta(seconds=30),
            ExecutionArtifactKind.RAW_INSTRUCTIONS,
            self.capabilities,
            raw_hash(payload)[:16],
        )
...
class OpenOceanAdapter(ProviderAdapter):
    ...
    def __init__(self, api_key: str | None = None, **kwargs: Any):
        self.capabilities = OPENOCEAN_CAPABILITIES if api_key else OPENOCEAN_DISABLED_CAPABILITIES
        super().__init__(**kwargs)
        self.api_key = api_key
    ...
    def normalize(self, req: QuoteRequest, payload: dict[str, Any]) -> NormalizedQuote:
        if str(payload.get("inAmount") or payload.get("inAmountBaseUnits")) != str(
            ...
            raise ValueError("openocean response does not match request")
        out = int(payload.get("outAmount") or payload.get("outAmountBaseUnits"))
        route = tuple(payload.get("sources") or payload.get("dexes") or ("openocean-meta",))
        fee = payload.get("fee") or "unknown"
        return NormalizedQuote(
            self.provider_id,
            req.fingerprint,
            raw_hash(payload),
            payload.get("traceId") or raw_hash(payload)[:16],
            req.input_mint,
            req.output_mint,
            req.amount_base_units,
            out,
            None,
            MinimumOutputState.UNPROVEN,
            req.swap_mode,
            req.slippage_bps,
            tuple(f"openocean:{item}" for item in route),
            tuple(str(item) for item in route),
            payload.get("priceImpact"),
            str(fee),
            (
                str(payload.get("platformFee"))
...
class OdosAdapter(ProviderAdapter):
    ...
    def quote_body(self, req: QuoteRequest) -> dict[str, Any]:
        return {
            ...
            "slippageLimitPercent": req.slippage_bps / 100,
        }
    ...
    def normalize_quote(self, req: QuoteRequest, payload: dict[str, Any]) -> NormalizedQuote:
        ...
        return NormalizedQuote(
            ...
            route,
            tuple(str(item) for item in route),
            str(payload.get("priceImpact")) if payload.get("priceImpact") is not None else None,
            str(payload.get("providerFee")) if payload.get("providerFee") is not None else None,
            str(payload.get("referralFee")) if payload.get("referralFee") is not None else None,
            None,
            self.clock.now(),
            self.clock.now() + timedelta(seconds=60),
            ExecutionArtifactKind.ASSEMBLED_TRANSACTION,
            self.capabilities,
            path,
        )
