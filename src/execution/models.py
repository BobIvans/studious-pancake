...
class Instruction:
    """String instruction descriptor retained only for non-active compatibility tests."""
...
@dataclass(frozen=True, slots=True)
class FlashLoanPlan:
    """String MarginFi descriptor retained only for non-active compatibility tests."""
...
class TransactionPlan:
    """Canonical Solders-only execution plan.

    PR-053 removes the previous positional compatibility constructor. Active
    callers must provide Solders ``Pubkey`` and ``PlannedInstruction`` values;
    the compiler rejects any string/compatibility objects before message build.
    """
