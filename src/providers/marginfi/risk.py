...
class MarginfiRiskAccountResolver:
    def resolve(
        ...
        out: list[str] = []
        seen: set[str] = set()
        for bank_key in sorted(
            margin_account.active_balances,
            key=lambda key: bytes(Pubkey.from_string(key)),
            reverse=True,
        ):
            bank = banks.get(bank_key)
            if bank is None or not bank.oracle_keys:
                raise MarginfiRejection(
                    MarginfiRejectionCode.ORACLE_INVALID,
                    "missing bank/oracle risk group",
                )
