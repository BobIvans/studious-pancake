...
class AggregatorClient:
    ...
    def __init__(self, name: str, quote_url: str, swap_url: Optional[str] = None, headers: Optional[Dict] = None):
        self.name = name
    ...
    async def get_quote(self, session: aiohttp.ClientSession, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def get_swap_instructions(self, session: aiohttp.ClientSession, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            url = self.swap_url
            if self.name == 'jupiter':
                # Swap instructions usually use a different proxy path, but we'll use the same proxies
                url = self.proxies[self.proxy_index % len(self.proxies)].replace('/quote', '/swap')
                self.proxy_index += 1
...
class MultiAggregatorClient:
    ...
    def __init__(self):
        ...
        self.jupiter_limiter = self._create_limiter(int(os.getenv("JUPITER_QUOTE_RPS", 5)))
        self.openocean_limiter = self._create_limiter(int(os.getenv("OPENOCEAN_RPS", 2)))
        self.okx_limiter = self._create_limiter(int(os.getenv("OKX_RPS", 5)))
        self.odos_limiter = self._create_limiter(int(os.getenv("ODOS_RPS", 1)))  # Strict 1 RPS
    ...
    async def get_quote(self, input_mint: str, output_mint: str, amount: int,
        ...
        if not self.session:
            raise RuntimeError("MultiAggregatorClient must be used as async context manager")
        ...
        output_mint = str(output_mint) if not isinstance(output_mint, str) else output_mint
        ...
        for _ in range(4):
            ...
            async with limiter:
                ...
                if aggregator.name == 'openocean':
                    ...
                    params = {
                        'chainId': 501,
                        'inputTokens': [{'tokenAddress': str(input_mint), 'amount': str(amount)}],
                        'outputTokens': [{'tokenAddress': str(output_mint), 'proportion': 1}],
    ...
    async def get_swap_instructions(self, quote_response: Dict[str, Any],
                                  user_public_key: str) -> Optional[Tuple[List[Dict], List[str]]]:
