...
def test_documentation_only_blocks():
    r = evaluate_admission(bundle(protected_workflow=False, credentialed_probe=False)).reason_codes
...
def test_missing_probe_and_runtime_port_block():
    b = bundle()
    assert "PROTECTED_PROBE_MISSING" in evaluate_admission(type(b)(**{**b.__dict__, "probe": None})).reason_codes
    assert "NO_ACTIVE_RUNTIME_PORT" in evaluate_admission(type(b)(**{**b.__dict__, "runtime_ports": ()})).reason_codes


def test_expiry_drift_credential_program_and_quorum_revoke():
    b = bundle()
    assert "EVIDENCE_EXPIRED" in evaluate_admission(type(b)(**{**b.__dict__, "now_unix_seconds": NOW + 4000})).reason_codes
    assert "CONTRACT_DRIFT_DETECTED" in evaluate_admission(type(b)(**{**b.__dict__, "contract_drift_detected": True})).reason_codes
    assert "CREDENTIAL_FAILURE" in evaluate_admission(type(b)(**{**b.__dict__, "credential_failure": True})).reason_codes
    assert "PROGRAM_IDENTITY_CHANGED" in evaluate_admission(type(b)(**{**b.__dict__, "program_identity_changed": True})).reason_codes
    assert "RPC_QUORUM_DISAGREED" in evaluate_admission(type(b)(**{**b.__dict__, "rpc_quorum_disagreed": True})).reason_codes
...
def test_jupiter_adapter_uses_v2_build_and_legacy_blocks():
    ...
    assert "JUPITER_LEGACY_ENDPOINT_PINNED" in evaluate_admission(bundle(pin=pin(endpoint="/swap/v1/quote"))).reason_codes
...
def test_solana_rpc_context_and_method_limits():
    req = SolanaRpcEvidenceService.rpc_request("https://rpc", "isBlockhashValid", ["h", {"minContextSlot": 5}])
...
def test_program_observation_and_marginfi_hash_required():
    obs = {"program_id": "p", "programdata_address": "pd", "executable": True, "owner": "loader", "slot": 1}
    ...
    assert "DEPLOYED_PROGRAM_OBSERVATION_MISSING" not in evaluate_admission(bundle(pin=p, observed_program_hash=H)).reason_codes
...
def test_solana_rpc_quorum_required():
    ...
    assert "RPC_QUORUM_HASH_MISSING" not in evaluate_admission(bundle(pin=p, rpc_quorum_hash=H)).reason_codes


def test_redacted_fixture_removes_secret_values(tmp_path):
    path = tmp_path / "f.json"
    h = write_redacted_fixture(path, ProviderName.JUPITER, {"Authorization": "Bearer abcdefghijklmnopqrstuvwxyz", "url": "https://x/?api_key=secret123"})
    text = path.read_text()
    assert len(h) == 64
...
def test_probe_plans_cover_provider_set_and_no_submission_or_legacy():
    ...
    assert {ProviderName.JUPITER, ProviderName.SOLANA_RPC, ProviderName.JITO_READONLY, ProviderName.MARGINFI} <= providers
...
def test_cli_plan_and_replay(tmp_path):
    ...
    plan = subprocess.run([sys.executable, "-m", "src.providers.conformance.mega_b2", "plan"], cwd=root, env=env, text=True, capture_output=True, check=True)
    assert "jupiter_build" in plan.stdout
    b = bundle()
    f = tmp_path / "bundle.json"
    f.write_text(json.dumps({"schema_version": b.schema_version, "contract_pin": b.contract_pin.__dict__, "probe": b.probe.__dict__, "runtime_ports": [p.__dict__ for p in b.runtime_ports], "now_unix_seconds": b.now_unix_seconds}, default=str))
    replay = subprocess.run([sys.executable, "-m", "src.providers.conformance.mega_b2", "replay", "--bundle", str(f), "--port", "jupiter_v2_build"], cwd=root, env=env, text=True, capture_output=True, check=True)
