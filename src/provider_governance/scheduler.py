...
class DeadlineAdmissionScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self._cleanup_error_types: deque[str] = deque(maxlen=16)
    ...
    async def _dispatch_locked(self) -> None:
        ...
        for item in sorted(
            ...
            self._pending.remove(item)
            if (
                len(self._served) >= self.max_queue_size
                and item.request.fairness_key not in self._served
            ):
                active = {queued.request.fairness_key for queued in self._pending}
                for key in tuple(self._served):
    ...
    def _cleanup_finished(self, task: asyncio.Task[None]) -> None:
        if task not in self._cleanup_tasks:
            return
        self._cleanup_tasks.discard(task)
        # Retrieve exceptions even when the original caller was cancelled and
        # can no longer await its shielded obligation cleanup.
        error = "CancelledError" if task.cancelled() else task.exception()
        if error is not None:
            self._cleanup_failures += 1
            self._cleanup_error_types.append(
                error if isinstance(error, str) else type(error).__name__
            )

    async def drain_cleanup(self, timeout: float) -> bool:
        """Bounded shutdown retains unresolved accounting in the durable owner."""
        if not self._cleanup_tasks:
            return self._cleanup_failures == 0
        done, pending = await asyncio.wait(tuple(self._cleanup_tasks), timeout=timeout)
        for task in done:
            self._cleanup_finished(task)
        return not pending and self._cleanup_failures == 0
    ...
    async def snapshot(self) -> dict[str, object]:
        async with self._condition:
            return {
                ...
                "cleanup_error_types": tuple(self._cleanup_error_types),
