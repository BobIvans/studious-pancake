...
def _provenance() -> KaminoDeploymentProvenance:
    return KaminoDeploymentProvenance(
        source_url=KLEND_SOURCE_REPOSITORY,
        sdk_package=KLEND_SDK_PACKAGE,
        lending_program_id=KLEND_PROGRAM,
        idl_sha256=_digest("klend-idl"),
...
def _combination(*, verified: bool = True) -> KaminoSupportedCombination:
    return KaminoSupportedCombination(
        combination_id="mainnet-sol-usdc-pr095",
...
def _base_artifact(kind: KaminoConformanceArtifactKind) -> KaminoConformanceArtifact:
    return KaminoConformanceArtifact(
        path=f"evidence/pr067/{kind.value}.json",
        sha256=_digest(f"pr067:{kind.value}"),
        kind=kind,
    )


def _base_artifacts() -> tuple[KaminoConformanceArtifact, ...]:
    return tuple(_base_artifact(kind) for kind in KaminoConformanceArtifactKind)
...
def _rpc_vector(
    ...
    *,
    ...
    return KaminoRpcAccountVector(
        ...
        account_kind=kind,
        data_sha256=_digest(f"rpc:{marker}:data"),
        decoded_fields_sha256=_digest(f"rpc:{marker}:decoded"),
        slot=100,
    )
...
def _rpc_vectors() -> tuple[KaminoRpcAccountVector, ...]:
    return (
        _rpc_vector(SYSTEM_PROGRAM_ADDRESS, KaminoAccountVectorKind.MARKET, "market"),
        _rpc_vector(TOKEN_PROGRAM_ADDRESS, KaminoAccountVectorKind.RESERVE, "sol"),
        _rpc_vector(TOKEN_2022_PROGRAM_ADDRESS, KaminoAccountVectorKind.RESERVE, "usdc"),
        _rpc_vector(
            COMPUTE_BUDGET_PROGRAM_ADDRESS,
...
def _instruction_vectors() -> tuple[KaminoInstructionGoldenVector, ...]:
    return (
        KaminoInstructionGoldenVector(
            ...
            account_metas_sha256=_digest("ix:refresh:metas"),
            data_sha256=_digest("ix:refresh:data"),
            account_count=8,
            writable_count=3,
            signer_count=0,
            sdk_fixture_sha256=_digest("ix:refresh:fixture"),
        ),
        KaminoInstructionGoldenVector(
            instruction_name="liquidate_obligation_and_redeem_reserve_collateral",
            program_id=KLEND_PROGRAM,
            account_metas_sha256=_digest("ix:liquidate:metas"),
            data_sha256=_digest("ix:liquidate:data"),
            account_count=14,
            writable_count=8,
            signer_count=1,
            sdk_fixture_sha256=_digest("ix:liquidate:fixture"),
        ),
    )
    ...
    *,
    ...
    return KaminoConformanceEvidence(
        ...
        planner_replay=KaminoPlannerReplayEvidence(
            ...
            corpus_sha256=_digest("planner-corpus"),
        ),
        shadow_soak=KaminoShadowSoakReference(
            run_id="pr095-kamino-shadow-soak-001",
...
def _real_artifact(kind: KaminoRealArtifactKind) -> KaminoRealArtifact:
    return KaminoRealArtifact(
        ...
        sha256=_digest(f"pr095:{kind.value}"),
        kind=kind,
...
def _package(
    *,
    base: KaminoConformanceEvidence | None = None,
) -> KaminoRealConformancePackage:
    conformance = base or _base_conformance()
    return KaminoRealConformancePackage(
        source_pins=_source_pins(),
        base_conformance=conformance,
        artifacts=_real_artifacts(),
        deployment_program_hash_sha256=_digest("deployment-program"),
        idl_sha256=conformance.combination.provenance.idl_sha256,
        ...
        rpc_evidence=KaminoRealRpcEvidence(
            market_vector_sha256=_digest("market-vector"),
            reserve_vector_sha256=_digest("reserve-vector"),
            obligation_vector_sha256=_digest("obligation-vector"),
            oracle_vector_sha256=_digest("oracle-vector"),
        ...
        shadow_soak=KaminoRealShadowSoakEvidence(
            run_id="pr095-kamino-shadow-soak-001",
            duration_seconds=MINIMUM_SHADOW_SOAK_SECONDS,
            evidence_sha256=conformance.shadow_soak.evidence_sha256,
...
def test_complete_pr095_package_is_shadow_review_ready_but_never_live() -> None:
    ...
    assert result.metrics_summary["rpc_reserves"] == 2


def test_unverified_pr067_base_blocks_real_conformance() -> None:
    base = _base_conformance(combination=_combination(verified=False))
    result = evaluate_kamino_real_conformance(_package(base=base))
...
def test_missing_pr095_artifact_kind_blocks_real_conformance() -> None:
    package = _package()
    result = evaluate_kamino_real_conformance(
        replace(package, artifacts=package.artifacts[:-1])
    )

    assert result.ready_for_shadow_review is False
    assert "PR095_REQUIRED_ARTIFACTS_MISSING" in result.blockers


def test_shadow_soak_hash_must_match_base_conformance() -> None:
    package = _package()
    result = evaluate_kamino_real_conformance(
        replace(
            package,
            shadow_soak=replace(package.shadow_soak, evidence_sha256=_digest("other")),
        )
    )

    assert result.ready_for_shadow_review is False
    assert "SHADOW_SOAK_HASH_MISMATCH" in result.blockers


def test_materialized_artifacts_must_exist_and_match_hash(tmp_path: Path) -> None:
    package = _package()
    for artifact in package.artifacts:
        path = tmp_path / artifact.path
        path.parent.mkdir(parents=True, exist_ok=True)
        marker = artifact.kind.value
        content = f"pr095:{marker}".encode("utf-8")
        assert hashlib.sha256(content).hexdigest() == artifact.sha256
        path.write_bytes(content)

    assert check_pr095_materialized_artifacts(package, tmp_path) == ()

    first_path = tmp_path / package.artifacts[0].path
    first_path.write_text("tampered", encoding="utf-8")
    blockers = check_pr095_materialized_artifacts(package, tmp_path)

    assert blockers == (f"ARTIFACT_HASH_MISMATCH:{package.artifacts[0].path}",)


def test_source_pins_must_be_official_and_non_placeholder() -> None:
...
def test_evaluation_dict_is_stable_json_serialisable() -> None:
    ...
    assert KLEND_SOURCE_REPOSITORY in encoded
