...
from solders.address_lookup_table_account import AddressLookupTable, AddressLookupTableAccount, LookupTableMeta, ADDRESS_LOOKUP_TABLE_ID
from solders.compute_budget import ID as COMPUTE_BUDGET_ID, set_compute_unit_limit, set_compute_unit_price
...
def transfer_ix(to=RECIPIENT, lamports=1, signer=None):
    return transfer(TransferParams(from_pubkey=(signer or PAYER.pubkey()), to_pubkey=to, lamports=lamports))


def memo_ix(data=b"opaque", extra=None):
    metas = [AccountMeta(PAYER.pubkey(), True, True)]
    if extra: metas.append(extra)
    return Instruction(Pubkey.new_unique(), bytes(data), metas)


def plan(*, ixs=None, cb=ComputeBudgetPolicy(400_000, 10), tip=TipPolicy(), signers=None, alts=(), required_lookup=()):
    return TransactionPlan("opp", PAYER.pubkey(), tuple(PlannedInstruction(ix, "application", "ix") for ix in (ixs or [transfer_ix()])), cb, tip, tuple(signers or (PAYER.pubkey(),)), tuple(alts), tuple(required_lookup), 7, 8, 9, (PAYER.pubkey(), RECIPIENT))
...
def resolved_alt(addresses=(RECIPIENT,), source_slot=50):
    return AltValidator().deserialize(ALT_KEY, alt_raw(addresses), ADDRESS_LOOKUP_TABLE_ID, source_slot, addresses)


def compile_default(**kwargs) -> CompiledTransaction:
    return TransactionCompiler().compile(plan(**kwargs), bh(), tuple(kwargs.get("lookup_tables", ())))
...
def test_compute_budget_and_tip_golden_ordering():
    ...
    assert bytes(c.instructions[-1]) == bytes(transfer(TransferParams(from_pubkey=PAYER.pubkey(), to_pubkey=tip_account, lamports=123)))
    no_tip = TransactionCompiler().compile(plan(tip=TipPolicy(0, tip_account)), bh())
    assert len(no_tip.instructions) == 3


@pytest.mark.parametrize("bad", [PlannedInstruction(set_compute_unit_limit(1), "application"), PlannedInstruction(transfer_ix(), "tip")])
def test_duplicate_compiler_owned_instructions_rejected(bad):
    p = TransactionPlan("opp", PAYER.pubkey(), (bad,), ComputeBudgetPolicy(400_000, 10), required_signers=(PAYER.pubkey(),))
...
@pytest.mark.parametrize("cb,tip", [(ComputeBudgetPolicy(0, 1), TipPolicy()), (ComputeBudgetPolicy(1_400_001, 1), TipPolicy()), (ComputeBudgetPolicy(1, -1), TipPolicy()), (ComputeBudgetPolicy(1, 1), TipPolicy(-1, None)), (ComputeBudgetPolicy(1, 1), TipPolicy(1, None))])
...
def test_sign_fully_single_and_message_hash_stable():
    ...
    assert bytes(to_bytes_versioned(s.versioned_transaction.message)) == c.serialized_message
    assert s.message_hash == c.message_hash


def test_multi_signer_requires_exact_set_and_rejects_bad_sets():
    other = kp(5)
    ix = Instruction(Pubkey.new_unique(), b"x", [AccountMeta(PAYER.pubkey(), True, True), AccountMeta(other.pubkey(), True, False)])
    c = TransactionCompiler().compile(plan(ixs=[ix], signers=(PAYER.pubkey(), other.pubkey())), bh())
    compiler = TransactionCompiler()
...
def test_unsigned_envelope_not_allowed_for_sigverify_simulation():
    class Rpc:
        async def call(self, method, params):
            if method == "getMultipleAccounts": return {"value": []}
            return {"context": {"slot": 1}, "value": {"err": None, "accounts": []}}
    with pytest.raises(ValueError):
        import asyncio; asyncio.run(TransactionSimulator(Rpc()).simulate(TransactionCompiler().compile(plan(), bh()), final_signed=True))
...
def test_alt_binary_fixture_parses_and_compiles_lookup():
    ...
    ix = Instruction(Pubkey.new_unique(), b"x", [AccountMeta(PAYER.pubkey(), True, True), AccountMeta(RECIPIENT, False, False)])
    c = TransactionCompiler().compile(plan(ixs=[ix], alts=(ALT_KEY,), required_lookup=(RECIPIENT,)), bh(), (alt,))
...
@pytest.mark.parametrize("raw,owner,source,required", [(b"a\nb", ADDRESS_LOOKUP_TABLE_ID, 50, ()), (alt_raw(()), ADDRESS_LOOKUP_TABLE_ID, 50, ()), (alt_raw((RECIPIENT, RECIPIENT)), ADDRESS_LOOKUP_TABLE_ID, 50, ()), (alt_raw((RECIPIENT,), deactivation=1), ADDRESS_LOOKUP_TABLE_ID, 50, ()), (alt_raw((RECIPIENT,), last_extended_slot=50), ADDRESS_LOOKUP_TABLE_ID, 50, ()), (alt_raw((RECIPIENT,)), Pubkey.new_unique(), 50, ()), (alt_raw((RECIPIENT,)), ADDRESS_LOOKUP_TABLE_ID, 50, (kp(9).pubkey(),))])
...
def test_signer_stays_static_even_if_in_alt():
    ...
    ix = Instruction(Pubkey.new_unique(), b"x", [AccountMeta(PAYER.pubkey(), True, True), AccountMeta(other.pubkey(), True, False)])
    c = TransactionCompiler().compile(plan(ixs=[ix], signers=(PAYER.pubkey(), other.pubkey()), alts=(ALT_KEY,)), bh(), (alt,))
    assert other.pubkey() in c.message.account_keys[: c.message.header.num_required_signatures]


def test_oversize_typed_error_has_diagnostics_and_counts_signatures():
    big_ixs = [memo_ix(bytes([i % 255]) * 90, AccountMeta(Pubkey.new_unique(), False, False)) for i in range(20)]
    with pytest.raises(TransactionCompileError) as e:
...
def test_determinism_and_mutations_change_hash():
    ...
    assert compiler.compile(p, bh()).message_hash == compiler.compile(p, bh()).message_hash
    assert compiler.compile(plan(ixs=[memo_ix(b"b")]), bh()).message_hash != compiler.compile(p, bh()).message_hash
    assert compiler.compile(plan(ixs=[Instruction(Pubkey.new_unique(), b"a", [AccountMeta(PAYER.pubkey(), True, False)])]), bh()).message_hash != compiler.compile(p, bh()).message_hash


def test_default_blockhash_and_string_boundary_rejected():
    with pytest.raises(TransactionCompileError):
        TransactionCompiler().compile(plan(), bh(Hash.default()))
    bad = TransactionPlan("opp", "payer", (), ComputeBudgetPolicy(1, 1), required_signers=("payer",))
    with pytest.raises(TransactionCompileError):
...
def test_rpc_shapes_for_simulate_and_fee_message():
    ...
    class Rpc:
        ...
        async def call(self, method, params):
            ...
            raw = base64.b64decode(params[0]); VersionedTransaction.from_bytes(raw).sanitize(); assert params[1]["sigVerify"] is False
            return {"context": {"slot": 11}, "value": {"err": None, "accounts": []}}
    import asyncio
    rpc = Rpc(); report = asyncio.run(TransactionSimulator(rpc).simulate(c, final_signed=False)); assert report.transaction_message_hash == c.message_hash
