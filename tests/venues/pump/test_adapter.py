import pytest
from src.venues.pump.adapter import PumpAdapter, PumpContractManifest, TOKEN_PROGRAM, TOKEN_2022_PROGRAM
from src.venues.pump.models import RawAccount, PumpFamily, SwapDirection, PumpLifecycle, ReasonCode
from src.ingest.pump_fun_predictor import PumpFunBondingCurve, PumpLegacyHeuristicDisabled, PUMP_LEGACY_HEURISTIC_DISABLED
...
def pk(ch): return (ch.encode()*32)[:32]


def bonding_data(complete=False):
    disc = bytes.fromhex('17b7f83760d8ac60')
    vals = [1_000_000, 2_000_000, 500_000, 800_000, 1_000_000_000]
    raw = disc + b''.join(v.to_bytes(8, 'little') for v in vals) + bytes([complete]) + pk('B') + pk('Q')
    return raw + bytes(137-len(raw))


def pool_data(virtual=0):
    disc = bytes.fromhex('f19a6d0411b16dbc')
    return disc + (500_000).to_bytes(8, 'little') + (800_000).to_bytes(8, 'little') + virtual.to_bytes(8, 'little') + pk('B') + pk('Q') + bytes(241-8-8*3-64)
...
def test_contract_detection_and_layout_fail_closed():
    ...
    with pytest.raises(ValueError, match=ReasonCode.PUMP_LAYOUT_SIZE_MISMATCH.value):
        a.decode_account(PumpFamily.BONDING_CURVE, acct(bonding_data()[:-1]))
...
def test_lifecycle_and_snapshot_slot_consistency():
    a = PumpAdapter()
    active = a.build_snapshot(PumpFamily.BONDING_CURVE, 'mint', (acct(bonding_data(False)),), {pk('B').hex(): TOKEN_PROGRAM})
    assert active.lifecycle is PumpLifecycle.BONDING_ACTIVE
    pending = a.build_snapshot(PumpFamily.BONDING_CURVE, 'mint', (acct(bonding_data(True)),), {})
    assert pending.lifecycle is PumpLifecycle.BONDING_COMPLETE_PENDING_DESTINATION
    confirmed = a.build_snapshot(PumpFamily.BONDING_CURVE, 'mint', (acct(bonding_data(True)),), {}, destination_verified=True)
    ...
    with pytest.raises(ValueError, match=ReasonCode.PUMP_MIXED_SLOT.value):
        a.build_snapshot(PumpFamily.BONDING_CURVE, 'mint', (acct(bonding_data(), slot=100), acct(bonding_data(), slot=99)), {})


def test_bonding_quote_and_instruction_order_shadow_only():
    a = PumpAdapter()
    snap = a.build_snapshot(PumpFamily.BONDING_CURVE, 'mint', (acct(bonding_data()),), {pk('B').hex(): TOKEN_PROGRAM})
    q = a.quote_exact_in(snap, SwapDirection.BUY, 1000, 1)
    assert q.executable_in_shadow and q.net_out_amount > 0
    metas = PumpContractManifest.load().by_family(PumpFamily.BONDING_CURVE).ordered_metas['buy_v2']
    accounts = {m: f'{m}_addr' for m in metas if m != 'user'}
    ix, _ = a.build_swap_ix(snap, q, accounts, 'user_addr')
    assert ix.kind == 'pump_shadow_swap' and ix.name == 'buy_v2'
    assert ix.accounts == tuple('user_addr' if m == 'user' else accounts[m] for m in metas)
...
def test_pumpswap_virtual_quote_reserves_affects_quote():
    a = PumpAdapter()
    s0 = a.build_snapshot(PumpFamily.PUMPSWAP, 'mint', (acct(pool_data(0), PumpFamily.PUMPSWAP),), {})
    s1 = a.build_snapshot(PumpFamily.PUMPSWAP, 'mint', (acct(pool_data(800_000), PumpFamily.PUMPSWAP),), {})
    q0 = a.quote_exact_in(s0, SwapDirection.BUY, 1000, 1)
    q1 = a.quote_exact_in(s1, SwapDirection.BUY, 1000, 1)
    assert q0.net_out_amount != q1.net_out_amount


def test_token_2022_policy_and_legacy_disabled():
    a = PumpAdapter()
    assert a.validate_token_policy(TOKEN_PROGRAM) is None
    assert a.validate_token_policy(TOKEN_2022_PROGRAM, ('metadata_pointer',), allow_token_2022=True) is None
    assert a.validate_token_policy(TOKEN_2022_PROGRAM, ('transfer_hook',),
                                   allow_token_2022=True) is ReasonCode.PUMP_UNSUPPORTED_TOKEN_EXTENSION
...
async def test_backrun_and_strategy_disabled_shadow_only():
    ...
    r = await BackrunTrigger(object()).on_migration_event(signature='sig', base_mint='b', quote_mint='q', recent_blockhash='bad')
