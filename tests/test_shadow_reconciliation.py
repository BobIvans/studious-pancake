...
def req():
    ...
    return SimulationRequest(
        ...
        "att",
        "p",
        __import__("hashlib").sha256(msg).hexdigest(),
        msg,
        1,
        ("payer",),
        compiler_diagnostics=CompilerDiagnostics(("payer", "vault"), ("altw",), ("altr",)),
    )
...
def rep(**kw):
    ...
    return SimulationReport(
        r,
        "https://rpc/?api-key=REDACTED",
        10,
        "3.1.8",
        kw.get("err"),
        tuple(kw.get("logs", ("repay ok",))),
        None,
        100,
        5000,
        {"writable": ("altw",), "readonly": ("altr",)},
        (10000, 0, 0, 0),
        (12000, 0, 0, 0),
        kw.get("pre_t", ()),
        kw.get("post_t", ()),
...
def test_fee_not_double_counted_and_repayment():
    out = ShadowReconciler(min_profit=1000).reconcile(
        rep(), required_repayment=1, observed_repayment=1
    )
...
def test_program_error_rejects():
    out = ShadowReconciler().reconcile(
        rep(err={"InstructionError": [0, "x"]}), required_repayment=1
    )
...
def test_loaded_addresses_mismatch_fails_closed():
    bad = rep()
    object.__setattr__(bad, "loaded_addresses", {"writable": ("wrong",), "readonly": ("altr",)})
...
def test_token_2022_integer_amount_and_uiamount_ignored():
    t = {
        ...
        "uiTokenAmount": {"amount": "12", "uiAmount": 999.9},
    }
